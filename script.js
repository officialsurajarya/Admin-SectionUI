...
function renderTable() {
        ...
        <tr>
            ...
            <td>
                <div class="d-flex justify-content-center gap-2">
                    <button class="btn btn-sm btn-primary view-btn px-3" style="width: 70px;" data-id="${user.id}">View</button>
                    <button class="btn btn-sm btn-warning edit-btn px-3" style="width: 60px;" data-id="${user.id}">Edit</button>
                    <button class="btn btn-sm btn-danger delete-btn px-3" style="width: 80px;" data-id="${user.id}">Delete</button>
                </div>
            </td>
        </tr>`).join("");
    ...
    document.querySelectorAll('.view-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            showUserModal(user); // opens the detail modal
        });
    });
...
function openEditModal(userId) {
    ...
    // ✅ Hide user details modal if it's visible
...
editUserForm.addEventListener('submit', function (e) {
    ...
    user.joinDate = document.getElementById('editJoinDate').value;
    ...
    // If the user is currently being viewed in the details modal, refresh that too
...
function renderPagination(totalPages) {
    ...
    const createBtn = (label, page = null, disabled = false, active = false) => {
    ...
    if (currentPage !== 1 && currentPage !== totalPages) {
        paginationContainer.appendChild(createBtn(currentPage, currentPage, false, true));
    }
...
exportBtn?.addEventListener("click", () => {
    ...
    const exportData = filteredData.slice(start, start + limit);

    const headers = [["ID", "Name", "Email", "Role", "Department", "Status", "Country", "Phone", "Age", "Join Date"]];
    const body = exportData.map(user => [
        user.id, user.name, user.email, user.role, user.department,
        user.status, user.country, user.phone, user.age, user.joinDate
    ]);

    doc.text("Admin Panel - Current Page Export", 14, 15);
    doc.autoTable({ startY: 20, head: headers, body: body, theme: "grid", styles: { fontSize: 8 } });
    doc.save("admin_table_page.pdf");
});

// Render Cards
function renderCards() {
    cardContainer.innerHTML = data.map(user => `
    ...
    const html = `
        <div class="row">
            <div class="col-md-4 text-center">
                <img src="${user.img}" alt="${user.name}" class="img-fluid rounded-circle mb-3" style="width: 120px; height: 120px;">
                <h5>${user.name}</h5>
            ...
            <div class="col-md-8">
                <ul class="list-group mb-3">
                    ...
                    <li class="list-group-item"><strong>Join Date:</strong> ${user.joinDate}</li>
                </ul>
